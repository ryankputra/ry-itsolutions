  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || user.balance < price) {
      Swal.fire({
        ...
        text: `Saldo akun Anda saat ini (Rp ${Number(user?.balance || 0).toLocaleString("id-ID")}) tidak mencukupi untuk biaya layanan ini (Rp ${price.toLocaleString("id-ID")}). Silakan lakukan Top Up Saldo terlebih dahulu.`,
        icon: "warning",
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2 pt-4 border-t border-hairline">
            <label className="text-xs font-bold text-ink flex items-center justify-between">
              ...
              <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full">
                Hanya Saldo Akun
              </span>
            </label>
            <div className="p-3.5 rounded-2xl border border-primary/30 bg-primary/5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-mono font-extrabold text-primary mt-0.5">
            ...
            {user && user.balance < getPrice(option) && (
              <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-300 text-xs flex items-center gap-2">
                ...
                <span>Saldo Anda kurang <b>Rp {(getPrice(option) - (user?.balance || 0)).toLocaleString("id-ID")}</b>. Silakan lakukan Top Up untuk menggunakan layanan ini.</span>
              </div>
            )}
          </div>
          ...
          >
            {submitting
              ...
              : `Bayar via Saldo Akun (Rp ${getPrice(option).toLocaleString('id-ID')})`}
          </Button>
        </form>
      </Card>
      ...
      {ceirResult && (
        <Card className="mt-4 p-5 border border-primary/30 bg-primary/5 animate-fade-in space-y-3">
          <h3 className="font-bold text-sm text-primary flex items-center gap-2">
            <svg className="w-3.5 h-3.5 inline mr-1 text-emerald-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z"/></svg> Hasil Pemeriksaan Server:
          </h3>
