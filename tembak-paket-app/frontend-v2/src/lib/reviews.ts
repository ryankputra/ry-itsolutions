export type ReviewRole = "Pembeli Terverifikasi" | "Pelanggan VIP" | "Konter Mitra";
...
export function getReviewRole(user: ReviewUser): ReviewRole {
  ...
  if ((user.userTotalOrders || 0) >= 10) return "Pelanggan VIP";
...
export function getReviewRoleClass(role: ReviewRole) {
  ...
  if (role === "Pelanggan VIP") return "bg-amber-50 text-amber-700 border-amber-200";
