  ...
  const renderServiceButtons = () => {
    ...
    Object.keys(ceirgoPricing).forEach(code => {
      ...
      if (price > 0) {
        ...
        const name = svc ? svc.name : code.replace(/_/g, ' ').toUpperCase();
...
fs.writeFileSync('src/app/(main)/cek-ceir/page.tsx', content);
console.log('CEIR User page updated dynamically');
