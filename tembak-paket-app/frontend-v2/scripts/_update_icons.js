...
c = c.replace(/\{ name: "Dashboard", href: "\/dashboard".*?\}/g, 
...
c = c.replace(/\{ name: "Unblock IMEI", href: "\/unblock-imei".*?\}/g, 
  \`{ name: "Unblock IMEI", href: "/unblock-imei", icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></svg> }\`
);
c = c.replace(/\{ name: "Cek CEIR", href: "\/cek-ceir".*?\}/g, 
...
c = c.replace(/\{ name: "Riwayat Transaksi", href: "\/history".*?\}/g, 
...
// Ganti icon install aplikasi
const oldInstallBtn = \`<button onClick={handleInstallPWA} className="w-full flex items-center justify-center gap-2 bg-primary/10 text-primary py-2.5 rounded-xl text-sm font-bold hover:bg-primary/20 transition-colors">
          📱 Install Aplikasi
        </button>\`;
const newInstallBtn = \`<button onClick={handleInstallPWA} className="w-full flex items-center justify-center gap-2 bg-primary text-white py-2.5 rounded-xl text-sm font-bold hover:bg-primary-focus transition-all shadow-sm">
...
c = c.replace(oldInstallBtn, newInstallBtn);

// Hapus emoji dari render span
c = c.replace(/<span className="text-lg">\{item.icon\}<\/span>/, '<span className={isActive(item.href) ? "text-primary" : "text-ink-muted"}>{item.icon}</span>');
