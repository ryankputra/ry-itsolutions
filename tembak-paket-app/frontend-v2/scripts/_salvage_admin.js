...
const p4 = \`
  ...
  const selectedPkg = selectedPkgIndex !== null && selectedPkgIndex >= 0 && selectedPkgIndex < packages.length ? packages[selectedPkgIndex] : null;
        ...
        <div className="space-y-6">
          <Card glass className="p-6 space-y-6 border-primary/50">
            <div className="flex justify-between items-start">
              ...
              <Button size="sm" variant="outline" onClick={loadManualData}>🔄 Refresh</Button>
            </div>
            
            {ceirgoServices.length === 0 ? (
              <div className="bg-amber-50 text-amber-800 p-4 rounded-xl text-sm border border-amber-200">
                Layanan Ceirgo tidak ditemukan. Pastikan CEIRGO_API_KEY di .env sudah benar.
              </div>
            ) : (
              <div className="space-y-4">
...
fs.writeFileSync('src/app/(main)/admin/page.tsx', p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9);
console.log('Successfully saved to disk using chunks!');