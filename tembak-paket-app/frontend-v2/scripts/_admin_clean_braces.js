...
const regex = /\{manualOrders\.filter\(o => !showOnlyPendingManual \|\| o\.status === 'pending' \|\| o\.status === 'processing'\)\.map\(o => \([\s\S]*?\}\)/m;

const replacement = \`{manualOrders.filter(o => !showOnlyPendingManual || o.status === 'pending' || o.status === 'processing').map(o => (
